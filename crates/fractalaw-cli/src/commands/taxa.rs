...
pub(crate) async fn cmd_taxa_classify(
    ...
    let weights_path = std::path::Path::new("crates/fractalaw-cli/config/drrp_classifier_v8.json");
    ...
    let actor_matcher = ActorMatcher::load("crates/fractalaw-core/data/actor-dictionary.yaml")
    ...
    for law_name in law_names {
        ...
        {
            let pos_weights = std::path::Path::new("crates/fractalaw-cli/config/position_classifier_v3.json");
