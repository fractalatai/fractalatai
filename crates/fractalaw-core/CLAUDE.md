...
- `actors.rs` — Actor matching from `docs/actor-dictionary.yaml` (compiled in via `include_str!`)
- `correlatives.rs` — Hohfeldian correlative inference from `docs/correlative-rules.yaml`
...
- Actor dictionary and correlative rules are **embedded at compile time** — the YAML files at `docs/actor-dictionary.yaml` and `docs/correlative-rules.yaml` must stay at those paths
