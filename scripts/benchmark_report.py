...
def run_report(category_filter=None):
    ...
    """)
    r = cur.fetchone()
    total, rp_ok, cp_ok, rd_ok, cd_ok, rp_t, cp_t, rd_t, cd_t = r
    ...
    print(f"  {'Metric':<20} {'Regex':>12} {'Classifier':>12}")
    print(f"  {'DRRP':<20} {pct(rd_ok,rd_t):>12} {pct(cd_ok,cd_t):>12}")
    print(f"  {'Position':<20} {pct(rp_ok,rp_t):>12} {pct(cp_ok,cp_t):>12}")
    ...
    print(f"  {'Category':<12} {'Total':>6} {'Regex':>10} {'Classifier':>12}")
    for cat, t, rok, cok in cur.fetchall():
        print(f"  {cat or 'unknown':<12} {t:>6} {pct(rok,t):>10} {pct(cok,t):>12}")

    # Position confusion matrices
    for tier, col in [("Regex", "regex_position"), ("Classifier", "cls_position")]:
        cur.execute(f"""
