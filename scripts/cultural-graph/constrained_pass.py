#!/usr/bin/env python3
"""Schema-constrained cultural graph extraction via Gemini.

Re-extracts narratives with forced edge types for SLM training data.
Supports multiple source types.

Usage:
    /usr/bin/python3 scripts/cultural-graph/constrained_pass.py --sample 5 --dry-run
    /usr/bin/python3 scripts/cultural-graph/constrained_pass.py --all
    /usr/bin/python3 scripts/cultural-graph/constrained_pass.py --input data/qq/cultural-graph/hazards.csv --id-prefix HZ --source-type hazard_report --all
"""
...
OUTPUT_DIR = DATA_DIR / "archive/constrained"
...
VALID_EDGE_TYPES = {
    ...
    "trusts", "works-around", "blames", "silences",  # absent from reporter-authored sources
    # "defers-to-by-rank" — dropped from schema (military-specific, 8 instances across 1200 narratives)
    "operational",
}
VALID_ENTITY_TYPES = {"People", "Plant", "Process", "Place", "Provision"}


def load_system_prompt():
    return (PROMPTS_DIR / "constrained-system-v1.md").read_text()
...
def call_gemini(system_prompt, narrative_text, site_code, api_key):
    ...
    user_prompt = (
        f"Site: {site_code}\n\n"
        f"Narrative:\n\"{narrative_text}\"\n\n"
        f"Extract entities and relationships. Classify each relationship "
        f"into exactly one edge type from the schema. "
...
def validate_extraction(result):
    """Validate constrained extraction result."""
    if "error" in result:
    ...
    for rel in result["relationships"]:
        if rel.get("edge_type") not in VALID_EDGE_TYPES:
            return False, f"invalid edge_type: {rel.get('edge_type')}"
    return True, "ok"


def main():
    parser = argparse.ArgumentParser(description="Schema-constrained cultural graph extraction")
    ...
    output_path = OUTPUT_DIR / f"constrained_{timestamp}.jsonl"

    # Load already-processed IDs for --resume
    done_ids = set()
    if args.resume:
        for p in OUTPUT_DIR.glob("constrained_*.jsonl"):
            with open(p) as f:
                for line in f:
                    try:
                        rec = json.loads(line)
        ...
        for i, row in enumerate(narratives):
            ...
            try:
                ...
                if valid:
                    ...
                    cultural = [r for r in rels if r.get("edge_type") != "operational"]
                    operational = len(rels) - len(cultural)
                    for r in rels:
                        et = r.get("edge_type", "unknown")
                        edge_type_counts[et] = edge_type_counts.get(et, 0) + 1
                    print(f"OK ({n_ent} ent, {len(cultural)} cultural, {operational} operational)")
                    stats["ok"] += 1
                else:
                    print(f"INVALID: {msg}")
                    stats["error"] += 1
