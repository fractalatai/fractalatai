#!/usr/bin/env python3
"""Approach F2: Gravity-weighted sum excluding strength.

Like F (drop strength entirely) but with gravity-dominant weighting on remaining 4.
Weights: gravity=0.40, scope_duty_bearer=0.25, scope_protected_class=0.25, hierarchy=0.10
Score: HIGH=3, MEDIUM=2, LOW=1, weighted sum, threshold into H/M/L.

Usage:
    /usr/bin/python3 scripts/significance_approach_f2.py
"""

import psycopg2

PG_DSN = "host=localhost port=5433 dbname=fractalaw user=fractalaw password=fractalaw"
APPROACH = "f2_gravity_no_strength"
...
WEIGHTS = {
    "gravity": 0.40,
    "scope_duty_bearer": 0.25,
    "scope_protected_class": 0.25,
    "hierarchy": 0.10,
}
...
def compute(sdb, spc, grav, strength, hier):
    vals = {"scope_duty_bearer": sdb, "scope_protected_class": spc, "gravity": grav, "hierarchy": hier}
