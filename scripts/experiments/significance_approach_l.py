#!/usr/bin/env python3
"""Approach L: Weighted score + size penalty.

Score = avg_significance * log2(total_provisions + 1).
Boosts larger laws — a 200-provision Act at avg 2.0 ranks above a 5-provision SI at 2.0.
Rating: percentile-based — top 20% -> HIGH, bottom 33% -> LOW, else MEDIUM.

Usage:
    /usr/bin/python3 scripts/significance_approach_l.py
    /usr/bin/python3 scripts/significance_approach_l.py --provision-approach b_gravity_weighted
"""
...
def main():
    ...
    APPROACH = f"l_weighted_size_{suffix}"
    ...
    for law_name, profile in laws.items():
        ...
        score = avg_sig * math.log2(total + 1)
    ...
    for law in ["UK_ukpga_1974_37", "UK_uksi_2015_51", "UK_uksi_1999_3242"]:
        if law in bench:
            ...
            print(f"  {law:<30s} {r[2]:>7s}  rank:{rank:>3}/{total_laws}  (score:{r[3]:.2f}, H:{r[4]} M:{r[5]} L:{r[6]} total:{r[7]})")

    print(f"\nTop 5 by weighted+size score:")
    for r in results[:5]:
        print(f"  {r[0]:<40s} score:{r[3]:>6.2f}  (H:{r[4]} M:{r[5]} L:{r[6]} total:{r[7]})")
    print(f"\nBottom 5:")
    for r in results[-5:]:
        print(f"  {r[0]:<40s} score:{r[3]:>6.2f}  (H:{r[4]} M:{r[5]} L:{r[6]} total:{r[7]})")
