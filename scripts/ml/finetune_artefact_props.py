#!/usr/bin/env python3
"""Fine-tune gemma-3-4b-it for JSP artefact property extraction on RunPod.

16-bit base, LoRA R=16. Extracts structured properties (owner, approver,
reviewer, frequency, content, criterion) from obligation text + artefact type.

IMPORTANT: Namespaced to /workspace/artefact-props/ on RunPod.

Setup on RunPod (A100 40GB+):
    pip install unsloth -q
    mkdir -p /workspace/artefact-props
    # Upload training data + this script
    python3 /workspace/artefact-props/finetune_artefact_props.py

Output:
    /workspace/artefact-props/output/gemma3-artefact-props-lora/
    /workspace/artefact-props/output/merged_model/
    /workspace/artefact-props/output/gemma3-artefact-props-q4.gguf
"""
...
TRAIN_FILE = "/workspace/artefact-props/train.jsonl"
TEST_FILE = "/workspace/artefact-props/val.jsonl"
OUTPUT_DIR = "/workspace/artefact-props/output/gemma3-artefact-props-lora"
MERGED_DIR = "/workspace/artefact-props/output/merged_model"
GGUF_NAME = "gemma3-artefact-props"
MAX_SEQ_LENGTH = 1024
EPOCHS = 3
BATCH_SIZE = 2
GRAD_ACCUM = 4
...
def preflight():
    print("=" * 60)
    print("PREFLIGHT — Artefact property extractor")
    print("=" * 60)
    ok = True
    import torch
...
def merge_and_export(model, tokenizer):
    ...
    os.remove(bf16)
    final = f"/workspace/artefact-props/output/{GGUF_NAME}-q4.gguf"
...
if __name__ == "__main__":
    ...
    print(f"\nDone. Load: ollama create {GGUF_NAME} -f /workspace/artefact-props/Modelfile")
