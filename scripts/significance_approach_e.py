#!/usr/bin/env python3
"""Approach E: Gravity + scope gate.

If gravity=HIGH AND (scope_duty_bearer>=MEDIUM OR scope_protected_class>=MEDIUM) -> HIGH
If gravity=LOW AND scope_duty_bearer=LOW AND scope_protected_class=LOW -> LOW
Else MEDIUM

Usage:
    /usr/bin/python3 scripts/significance_approach_e.py
"""

import psycopg2

PG_DSN = "host=localhost port=5433 dbname=fractalaw user=fractalaw password=fractalaw"
APPROACH = "e_gravity_scope_gate"
...
def compute(sdb, spc, grav, strength, hier):
    ...
    if grav == "HIGH" and (sdb_r >= 2 or spc_r >= 2):
        return 3.0, "HIGH"
    if grav == "LOW" and sdb_r <= 1 and spc_r <= 1:
        return 1.0, "LOW"
    return 2.0, "MEDIUM"
