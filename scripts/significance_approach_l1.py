#!/usr/bin/env python3
"""Approach L1: Weighted sum of avg_significance and log2(size).

Instead of multiplying (L), use a weighted sum for more control:
  score = w1 * avg_sig + w2 * log2(total+1)
  w1=0.6, w2=0.4 — importance matters more than size

Rating: percentile-based — top 20% -> HIGH, bottom 33% -> LOW, else MEDIUM.

Usage:
    /usr/bin/python3 scripts/significance_approach_l1.py
    /usr/bin/python3 scripts/significance_approach_l1.py --provision-approach b_gravity_weighted
"""
...
W1 = 0.6  # weight for avg significance
W2 = 0.4  # weight for log2(size)
...
def main():
    ...
    for law_name, profile in laws.items():
        ...
        if total == 0:
            continue
        avg_sig = (3 * profile["HIGH"] + 2 * profile["MEDIUM"] + 1 * profile["LOW"]) / total
        # Normalise log2 to roughly same scale as avg_sig (1-3)
        # log2(max_total+1) ~ 10 for 754 provisions, so divide by ~3.3 to scale to 0-3
        log_size = math.log2(total + 1) / 3.3
        score = W1 * avg_sig + W2 * log_size
    ...
    for law in ["UK_ukpga_1974_37", "UK_uksi_2015_51", "UK_uksi_1999_3242"]:
        if law in bench:
            ...
            print(f"  {law:<30s} {r[2]:>7s}  rank:{rank:>3}/{total_laws}  (score:{r[3]:.3f}, H:{r[4]} M:{r[5]} L:{r[6]} total:{r[7]})")

    print(f"\nTop 5:")
    for r in results[:5]:
        print(f"  {r[0]:<40s} score:{r[3]:>.3f}  (H:{r[4]} M:{r[5]} L:{r[6]} total:{r[7]})")
    print(f"\nBottom 5:")
    for r in results[-5:]:
        print(f"  {r[0]:<40s} score:{r[3]:>.3f}  (H:{r[4]} M:{r[5]} L:{r[6]} total:{r[7]})")
